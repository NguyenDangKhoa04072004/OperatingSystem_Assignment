...
#define PAGING_SWP_MASK  GENMASK(25,5)
...
#define PAGING_FPN_PRESENT(x)  GETVAL(x,PAGING_FPN_MASK,0)
/* Extract SWAPFPN */
#define PAGING_PGN(x)  GETVAL(x,PAGING_PGN_MASK,PAGING_ADDR_PGN_LOBIT)
/* Extract SWAPTYPE */
#define PAGING_FPN_SWAP(x)  GETVAL(x,PAGING_SWP_MASK,5)
...
int vmap_page_range(struct pcb_t *caller, int addr, int pgnum, int swap,
...
int alloc_pages_range(struct pcb_t *caller, int incpgnum, struct framephy_struct ** alloc_frame);
...
/* MEM/PHY protypes */
int MEMPHY_get_swap_page(struct memphy_struct *mp, int *offset);
